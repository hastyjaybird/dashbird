...
export function resolveCalendarEmbedUrl(env = process.env) {
  ...
  return embedUrlFromCalendarSrc(src, resolveCalendarTimeZone(env));
}
...
export async function fetchUpcomingGoogleCalendarEvents(env = process.env) {
  ...
  if (cache && cache.key === key) {
    ...
    if (fresh) {
      return calendarSuccessPayload(icalUrl, tz, cache.events, { cached: true });
    }
    ensureCalendarCacheRefresh(icalUrl, tz);
    return calendarSuccessPayload(icalUrl, tz, cache.events, { cached: true, stale: true });
  }
  ...
  if (cache && cache.key === key) {
    return calendarSuccessPayload(icalUrl, tz, cache.events, { cached: false });
  }
  return result;
}
